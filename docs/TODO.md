...
- 自己画的 24 个线稿动画表情（心情 / 干劲 / 在路上 / 日常），SVG + Web Animations，亮暗主题都跟着变
- 源文是 `:otw_<id>:` 短码；编辑器里像一个字符（贴着不展开、退格整删、方向键跨过），单击重播、双击改源码
- `Mod-e` 选择器：贴光标、最近使用、中文 / 拼音 / 英文搜索、方向键 + 回车、输入法组字不误触
- 列表摘要、目录、表格单元格里换成 Unicode；主包只带一张 24 项的小表
- 行高零变化、在前面打字不重播、同时自动播放最多 12 个；细节见技术方案 §11.4.2
- 性能：静止时换成一张按主题缓存的图，只在播放时叠上活的 SVG —— 一屏 400 个表情首次渲染 217 → 103ms，画质逐像素验证未变
